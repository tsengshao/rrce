...
Data sources:
- x axis/color: axisy_ctrl_daily_profiles.nc (produced by cal_axisy_ctrl_daily.py)
- y axis: one or more case-aligned NetCDF files, default axisy_exp_daily_profiles.nc
"""
...
def main(
    ...
    for y_path, source_name, source_marker in zip(y_nc_paths, y_source_names, y_markers):
        ...
        dry_fraction_ctrl = ctrl_data_for_exps(ctrl_ds, exp_vals, ctrl_days, "dry_fraction", method)
        ...
        c_data = dry_fraction_ctrl.transpose("exp").values.astype(float)
        face_colors = cmap(norm(c_data))
        edge_colors = np.array([(r * 0.7, g * 0.7, b * 0.7, 1.0) for r, g, b, a in face_colors])
        case_day = (
            ds["case_day"].sel(exp=exp_vals).values.astype(float)
            if "case_day" in ds.coords
            else ctrl_days
        )
...
if __name__ == "__main__":
    # Default usage:
    # - x/color reads axisy_ctrl_daily_profiles.nc
    # - y reads axisy_exp_daily_profiles.nc
    #
    # Multiple y nc files:
    # main(
    #     y_nc_paths=[
