...
def main(
    ...
    for exp in exp_vals[1:]:
        ...
        cwv_init = to_vtype_radius(ds["cwv"].sel({"period": "init", "method": method, "exp": exp}))

        fig = plt.figure(figsize=(10*1.2, 6*1.2))
        #ax = fig.add_axes([0.05, 0.15, 0.9, 0.7])
        ax = fig.add_axes([0.12, 0.15, 0.8, 0.7])
        udraw.draw_pannel_3vars(ax, radius_km, tw_init, rw_init, cwv_init)
        ...
        cwv_last = to_vtype_radius(ds["cwv"].sel({"period": "last", "method": method, "exp": exp}))

        fig = plt.figure(figsize=(10*1.2, 6*1.2))
        #ax = fig.add_axes([0.05, 0.15, 0.9, 0.7])
        ax = fig.add_axes([0.12, 0.15, 0.8, 0.7])
        udraw.draw_pannel_3vars(ax, radius_km, tw_last, rw_last, cwv_last)
