...
tsppdy = int(tspdy*5)
npday = int((nt-1)/tsppdy)+1  #total penta days 23, 22/5=4...2
print(nday, nt, tspdy, npday)

outdir=config.dataPath+f"/mspace_penta/{exp}/"
print(outdir)
...
idayTS, idayTE = tools.get_mpi_time_span(0, npday, cpuid, nproc)
print(cpuid, idayTS, idayTE, idayTE-idayTS)

dataWriter = DataWriter(outdir)
for iday in range(idayTS, idayTE):
  idxTS, idxTE = (iday-1)*tsppdy+1, iday*tsppdy+1
  if iday==0:
    idxTS, idxTE = 0, 1
  nidxt = idxTE-idxTS
  ...
  dataWriter.toNC(f"mspace_cloud-{iday:03d}penta.nc", \
    data=vardict,
    coords=dict(
      time=[it],
      zc=(['zc'], zc),
