...
if cpuid==0:
  _ = axisy.create_axmean_ctl_from_axisy_ctl(outdir, exp, append_text='_ano')


for it in range(it_start, it_end):
  nc_axsy   = Dataset(f'{outdir}/axisy-{it:06d}.nc', 'r')
  fname_out = f'{outdir}/axmean_ano-{it:06d}.nc'
  ...
  for varname in nc_axsy.variables.keys():
    ...
    data = in_ncvar[:]
    data = data - np.mean(data, axis=(axis_theta,axis_theta+1), keepdims=True)
