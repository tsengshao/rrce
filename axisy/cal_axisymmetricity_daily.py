...
for idy in range(idy_start, idy_end):
  print(cpuid, idy)
  nc_axsy0   = Dataset(f'{outdir}/axisy-{idy*day2num:06d}.nc', 'r')
  fname_out = f'{outdir}/axmean_daily-{idy:06d}.nc'
  nc_out  = axisy.create_nc_copy_dims(fname=fname_out, src=nc_axsy0, kick_out_members=['theta'])
  _       = axisy.add_dims_into_nc(nc_out, 'vtype', vtype, ('vtype'), {'num0':'mean', 'num1':'axisymmetricity'})
 
  for varname in nc_axsy0.variables.keys():
    if varname in nc_axsy0.dimensions.keys(): continue
    in_ncvar   = nc_axsy0.variables[varname]
    data = np.zeros(in_ncvar[:].shape)
    for it in range(idy*day2num, (idy+1)*day2num):
        nc_axsy   = Dataset(f'{outdir}/axisy-{it:06d}.nc', 'r')
        data += nc_axsy.variables[varname][:] / day2num
        nc_axsy.close()
